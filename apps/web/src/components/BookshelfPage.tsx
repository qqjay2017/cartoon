    ...
    return (
      <Card>
        <CardContent className="py-12 text-center text-muted-foreground">
          书架还是空的，
          <Link to="/config" className="text-primary underline-offset-4 hover:underline">去添加书籍</Link>
        </CardContent>
      </Card>
    )
  }
          ...
          <Card key={item.id} className="overflow-hidden">
            <Link to="/book/$bookshelfId" params={{ bookshelfId: item.id }}>
              <div className="aspect-[3/4] bg-muted flex items-center justify-center overflow-hidden">
                {item.coverUrl
                  ? <img src={proxyImage(item.coverUrl)} alt={item.name} className="h-full w-full object-cover" />
                  : <span className="text-4xl text-muted-foreground">{item.name.slice(0, 1)}</span>}
              </div>
            </Link>
            <CardHeader className="pb-2">
              ...
              <p className="text-xs font-mono text-muted-foreground/80">{item.id}</p>
            </CardHeader>
            ...
            <CardFooter className="gap-2 flex-wrap">
              <Button variant="default" size="sm" asChild>
                <Link to="/book/$bookshelfId" params={{ bookshelfId: item.id }}>详情</Link>
              </Button>
              <Button variant="secondary" size="sm" asChild>
                <Link to={readTo}><BookOpen className="size-3.5" />继续</Link>
              </Button>
